...
{
    $fileName = 'data_lansia_mekarjaya_' . date('Y-m-d') . '.csv';
    $lansias = \App\Models\Lansia::all();
    ...
    $columns = array('ID', 'Nama', 'NIK', 'Alamat', 'RW', 'Penyakit', 'Dibuat Pada');

    $callback = function() use($lansias, $columns) {
        $file = fopen('php://output', 'w');
        fputcsv($file, $columns);

        foreach ($lansias as $lansia) {
            $row['ID']  = $lansia->id;
            $row['Nama']    = $lansia->nama;
            $row['NIK']    = $lansia->nik;
            $row['Alamat']  = $lansia->alamat;
            $row['RW']  = $lansia->rw;
            $row['Penyakit']  = $lansia->penyakit;
            $row['Dibuat Pada']  = $lansia->created_at;

            fputcsv($file, array($row['ID'], $row['Nama'], $row['NIK'], $row['Alamat'], $row['RW'], $row['Penyakit'], $row['Dibuat Pada']));
        }
